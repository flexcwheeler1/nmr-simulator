...
sys.path.append(os.path.dirname(os.pat        except Exception as e:
            self._log_message(f"Error loading demo data: {str(e)}")
            messagebox.showerror("Load Error", f"Failed to load demo data:\\n{str(e)}")
    
    def _load_real_sdbs(self):
        """Load real SDBS data from the database."""
        sdbs_id = self.sdbs_id_entry.get().strip()
        
        if not sdbs_id:
            messagebox.showwarning("No SDBS ID", "Please enter an SDBS ID to load real data.")
            return
        
        self._log_message(f"Loading real SDBS data for ID: {sdbs_id}")
        
        try:
            # Use the real SDBS scraper
            spectrum_data = self.real_scraper.get_nmr_data(sdbs_id)
            
            if not spectrum_data:
                self._log_message(f"No data found for SDBS ID: {sdbs_id}")
                messagebox.showwarning("No Data", f"No NMR data found for SDBS ID: {sdbs_id}")
                return
            
            # Clear previous spectra from viewer
            self.spectrum_viewer.clear_spectra()
            
            # Create spectrum from real data
            spectrum = Spectrum(
                title=f"SDBS {sdbs_id} - {spectrum_data.get('compound_name', 'Unknown')}",
                nucleus="1H",
                ppm_range=(0, 12)
            )
            
            # Add peaks from real data
            peaks_data = spectrum_data.get('peaks', [])
            if peaks_data:
                for peak_info in peaks_data:
                    from nmr_simulator.spectrum import Peak
                    peak = Peak(
                        chemical_shift=peak_info.get('ppm', 0),
                        intensity=peak_info.get('intensity', 1.0),
                ...
                self._log_message(f"Loaded real SDBS data: {len(peaks_data)} peaks")
                
            else:
                self._log_message("Real SDBS data loaded but no peaks found")
                messagebox.showinfo("Data Loaded", f"SDBS data loaded for {spectrum_data.get('compound_name', 'Unknown')}, but no peak data available.")
            
        except Exception as e:
            self._log_message(f"Error loading real SDBS data: {str(e)}")
            messagebox.showerror("SDBS Load Error", f"Failed to load real SDBS data:\\n{str(e)}")
    
    def _simulate_spectrum(self):name(os.path.abspath(__file__))))

from nmr_simulator import NMRSimulator, Molecule, Spectrum
from sdbs_import import SDBSScraper, SDBSParser
from sdbs_import.real_scraper import RealSDBSScraper
...
class NMRSimulatorGUI:
    ...
    def __init__(self):
        ...
        self.real_scraper = RealSDBSScraper()  # Real SDBS scraper
        
        self.current_molecule: Optional[Molecule] = None
        self.current_spectra: List[Spectrum] = []
        self.current_peaks: List[Dict] = []  # Store peak data for manual annotation
    ...
    def _setup_left_panel(self):
        ...
        # Mode selection
        mode_frame = ttk.Frame(sdbs_frame)
        mode_frame.pack(fill=tk.X, pady=2)
        
        self.sdbs_mode_var = tk.StringVar(value="demo")
        ttk.Radiobutton(mode_frame, text="Demo Mode", 
                       variable=self.sdbs_mode_var, value="demo").pack(side=tk.LEFT)
        ttk.Radiobutton(mode_frame, text="Real SDBS", 
                       variable=self.sdbs_mode_var, value="real").pack(side=tk.LEFT, padx=(10, 0))
        
        # Real SDBS ID input
        sdbs_id_frame = ttk.Frame(sdbs_frame)
        sdbs_id_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(sdbs_id_frame, text="SDBS ID:").pack(side=tk.LEFT)
        self.sdbs_id_entry = ttk.Entry(sdbs_id_frame, width=10)
        self.sdbs_id_entry.pack(side=tk.LEFT, padx=(5, 5))
        self.sdbs_id_entry.insert(0, "1841")  # Indole example
        
        ttk.Button(sdbs_id_frame, text="Load Real Data", 
                  command=self._load_real_sdbs).pack(side=tk.LEFT)
        
        # Demo search
        demo_frame = ttk.Frame(sdbs_frame)
        demo_frame.pack(fill=tk.X, pady=2)
        
        ttk.Button(demo_frame, text="Search Demo SDBS", 
                  command=self._search_sdbs).pack(side=tk.LEFT, padx=(0, 5))
        
        ttk.Button(demo_frame, text="Load Demo Data", 
                  command=self._load_demo_data).pack(side=tk.LEFT)
    ...
    def _search_sdbs(self):
        ...
        try:
            ...
            results = self.scraper.search_compound(compound_name)
            
            # Clear previous results
            self.results_listbox.delete(0, tk.END)
            
            # Add results to listbox with enhanced display
            for result in results:
                category = result.get('category', 'Unknown')
                display_text = f"{result['name']} ({result.get('formula', 'Unknown')}) - {category}"
                self.results_listbox.insert(tk.END, display_text)
            
            if results:
                self._log_message(f"Found {len(results)} compounds matching '{compound_name}'")
            else:
                self._log_message(f"No compounds found for '{compound_name}'. Try 'alcohol', 'aromatic', 'ketone', etc.")
    ...
    def _load_demo_data(self, compound_name: Optional[str] = None):
    ...
    def _load_real_sdbs(self):
        ...
        try:
            ...
            if peaks_data:
                ...
                # Store peak data for manual annotation
